      ...
      const response = await axios.post(
        "https://api.openai.com/v1/chat/completions",
        {
          ...
          messages: [
            ...
            {
              role: "user",
              content: `Give me a simple explanation in Arabic about ${searchTerm}, if the ${searchTerm} is a medical term write including its description, what happens if it's too high or too low, and related symptoms. Arrange the output like this with sections separated by ###:
                        ### الوصف:
                        - محتوى الوصف
                        ...
                        if the ${searchTerm} not a medical term said it's not a medical term`,
            },
          ],
          temperature: 0,
        },
    ...
    <div className="flex flex-col min-h-screen bg-custom-gradient">
      ...
      <div className="flex-grow container mx-auto w-full text-center my-10 px-4">
        <Title
          title={
            <span>
              ابحث عن <span className="highlight">المصطلح الطبي</span> تود
              المعرفة عنه
            </span>
          }
          linkPath="/"
        ...
        <div className="flex flex-col items-center p-6 space-y-6 mt-4">
          ...
          {/* {loading && <p className="text-3xl text-gray-400">جاري البحث...</p>} */}

{/* Info Box */}
{!result ? (
  <div className="flex flex-col items-center"> {/* Center align the content */}
    <img src={emptySearchIcon} alt="No results found" className="h-[330px] object-cover mb-4" /> {/* Added margin for spacing */}
    
    {/* Conditionally render the <h1> based on loading state */}
    <h1 className="text-3xl text-gray-400">
      {loading ? 'جاري البحث...' : 'لم يتم البحث'}
    </h1>
  </div>
) : (
  <InfoBox
